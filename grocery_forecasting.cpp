...
int main ()
{
    ...
    const double PROMOTION_BOOST = 0.10;
	...
	choice = displayMenu(); // function 1
        ...
        if (choice == 1)
        {
            ...
            // Predictions depend on whether the product will be discounted next month
            ...
            if (runPromo == 'y' || runPromo == 'Y') {
                ...
                predictedQuantity = (currentQuantity * (1 + growthRate / 100)) * (1 + 			PROMOTION_BOOST);
...
// 4. Returns growth rate percentage
double calculateGrowthRate(int previousQuantity, int currentQuantity, double DEFAULT_GROWTH_RATE) {
